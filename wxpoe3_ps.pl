...
sub start_display {
	...
	print "start display [$self] frame[$wxframe]\n";
...
sub push_new_signal {
    ...
    if( @_ ) { 
		...
		print "signal key[$sigkey] val[$sigvalue]\n";
        push @$sq, $sig;
        return 1;
    }
...
sub signal_registration {
    ...
    if( @_ ) { 
		...
		print "push registration for [$sigkey]\n";
...
use POE::Loop::Wx;
use POE::Session;
use Timing::Dev::POE::Component::WxPoeIO;
...
#                                $sig_args->{$sigkey} = 1;
print "frame register 1-- sigkey[$sigkey] size[".scalar(keys %$sig_args)."]\n";
                                $sig_args->{SIGNAL_KEY} = $sigkey;
                                $sig_args->{WXFRAME_OBJ} = $_;
                                if( exists $sig_args->{INTER_FRAME} and $sig_args->{INTER_FRAME} ) {
print "frame register 2 to 2 -- sigkey[$sigkey] size[".scalar(keys %$sig_args)."] [$sig_args]\n";
                                    $_[KERNEL]->post( 'WxPoeIO', 'REGISTER_FRAME_TO_FRAME', $sig_args);
                                } else {
print "frame register 2-- sigkey[$sigkey] size[".scalar(keys %$sig_args)."] [$sig_args]\n";
...
#                $sig_args->{$sigkey} = 1;
print "pre-register-- sigkey[$sigkey] size[".scalar(keys %$sig_args)."]\n";
                $sig_args->{SIGNAL_KEY} = $sigkey;
                if( exists $sig_args->{INTER_FRAME} and $sig_args->{INTER_FRAME}) {
                    # INTER_FRAME indicates that the Poe registration process can be skipped
                    next;
                }
print "poe register-- sigkey[$sigkey] size[".scalar(keys %$sig_args)."]\n";
        ...
        pulse => sub {
            if (@MyApp::frames) {
                ...
                $_[KERNEL]->delay( pulse => 3 );
						...
						if(scalar(@$signal_queue)) {
							print "got a signal! [@$signal_queue]\n";
						}				
                # Manage signals!
