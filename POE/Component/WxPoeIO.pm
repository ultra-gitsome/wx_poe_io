...
sub Config_signal {
	...
	print "config signal [".$args->{SIGNAL_KEY}."] size[".scalar(keys %$args)."] [$args]\n";
	...
	print "using WXPOEIO_CHANNELS channel [".$loc_args{SIGNAL_CHANNEL}."] for signal [".$args->{SIGNAL_KEY}."] [".$_[HEAP]->{WXPOEIO_CHANNELS}->{$loc_args{SIGNAL_CHANNEL}}."]\n";
...
sub Register_session {
	...
	print "registering session [".$args->{SIGNAL_KEY}."]\n";
	# Validation - silently ignore errors
	if ( ! defined $args->{SIGNAL_KEY} ) {
		if ( DEBUG ) {
			warn 'Did not get any arguments';
...
#	# register within the WXPOEIO hash structure
	...
	print "all registered for SignalKey: [".$args->{SIGNAL_KEY}."]\n";
	# All registered!
	return 1;
}
	...
	if ( exists $_[HEAP]->{WXPOEIO_QUEUE} ) {
		...
		while( scalar(@$sq) ) {
			...
			foreach my $sigkey (keys %$signal) {
				...
				print "[trigger] sigkey[$sigkey] val[$sigvalue]\n";
				...
				print "[trigger] sending sigkey[$sigkey] to_poe\n";
...
sub toPoe {
	...
	print "[to_poe] check signal sigkey[$sigkey] val[$sigvalue]\n";
	...
	if ( exists $_[HEAP]->{WXPOEIO}->{ $sigkey } ) {
		...
		print "[to_poe] check latch sigkey[$sigkey] val[$sigvalue]\n";
		...
		print "[to_poe] no latching on sigkey[$sigkey] val[$sigvalue]\n";
		...
		print "[to_poe] no latching is set on sigkey[$sigkey] val[$sigvalue]\n";
...
sub _manage_to_poe {
	...
	print "using channel [$channel] for signal [$sigkey]\n";
...
#			return;
	...
	print "using WXPOEIO_CHANNELS channel [$channel] for signal [$sigkey] [".$_[HEAP]->{WXPOEIO_CHANNELS}->{$channel}."]\n";
	...
	foreach my $TargetSession ( keys %{ $_[HEAP]->{WXPOEIO}->{ $sigkey } } ) {
		...
		if ( ! $_[KERNEL]->ID_id_to_session( $TargetSession ) ) {
			...
			if ( DEBUG ) {
				...
				if(defined $_[KERNEL]->alias_resolve($TargetSession)) {
					...
					warn "TargetSession ID is [".$ts."]";
					$PSession = $_[KERNEL]->ID_session_to_id( $ts );
				warn "new TargetSession ID [$PSession] has been found";
				}
		} else {
		...
		print "send signal [$sigkey] to post[$PSession] target[$TargetSession] [$evt_meth]\n";
#		if($TargetSession==1) {
#				$_[KERNEL]->yield(	$evt_meth,$sigkey,$sigvalue	);
#				next;
#		}
...
sub _wait_on_latch {
	...
	print "wait on latch - latch count [".$_[HEAP]->{SIGNAL_KEY_HREF}->{$sigkey}->{LATCH_ATTEMPTS}."]\n";
		$_[KERNEL]->yield('_MANAGE_TO_POE', $sigkey, $sigvalue);
